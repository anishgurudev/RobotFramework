*** Settings ***
Documentation       to validate the login page objects objects

Library             SeleniumLibrary
Library             Collections
Library             ../customLibraries/Shop.py
Resource            ../resource/resource.robot
Resource            ../PageUtils/LoginPage.robot
Resource            ../PageUtils/ShopPage.robot

Suite Setup         Launch the browser
# Suite Teardown    Close Browser Session
Test Setup          open the url


*** Variables ***
@{Products}     Blackberry


*** Test Cases ***
Validate unSuccessful Login
    LoginPage.Fill the login Form    ${user_name}    ${invalid_password}
    Resource.wait until element is visible on screen    ${ErrorMessage_Xpath}
    LoginPage.verify error message is correct

Validate Cards Display in the Shopping page
    LoginPage.Fill the login Form    ${user_name}    ${valid_password}
    resource.wait until element is visible on screen    ${ShopPage_Checkout_Btn}
    ShopPage.verify Card titles in the shop page
    #scroll till Element    xpath:(//*[@class='card-footer'])[4]/button
    ShopPage.select the cards    ${Products}
 #    Add item to cart and checkout    Nokia Edge

    #Hello Test
    resource.scroll till Element    ${ShopPage_Checkout_Btn}

Select the form and navigate to child window
    LoginPage.Fill tthe login details and select the user option    ${user_name}    ${valid_password}
